...
def replace_ids(file_path):
    """Replace all IDs in the HTML file according to the mapping."""
    ...
    for old_id, new_id in ID_MAPPING.items():
        ...
        # for="value"
        pattern = f'for="{old_id}"'
        if pattern in content:
            new_pattern = f'for="{new_id}"'
            count = content.count(pattern)
            content = content.replace(pattern, new_pattern)
            replacement_count += count
        ...
        # data-* attributes with id value
        pattern = f'data-id="{old_id}"'
        if pattern in content:
            new_pattern = f'data-id="{new_id}"'
            count = content.count(pattern)
            content = content.replace(pattern, new_pattern)
            replacement_count += count
        
        # aria-labelledby and similar
        pattern = f'aria-labelledby="{old_id}"'
        if pattern in content:
            new_pattern = f'aria-labelledby="{new_id}"'
            count = content.count(pattern)
            content = content.replace(pattern, new_pattern)
            replacement_count += count
        
        # In JavaScript references like getElementById('id')
        pattern = f"getElementById('{old_id}')"
        if pattern in content:
            new_pattern = f"getElementById('{new_id}')"
            count = content.count(pattern)
            content = content.replace(pattern, new_pattern)
            replacement_count += count
        ...
        pattern = f"querySelector('#{old_id}')"
        if pattern in content:
            new_pattern = f"querySelector('#{new_id}')"
            count = content.count(pattern)
            content = content.replace(pattern, new_pattern)
            replacement_count += count
        ...
        # querySelector with various notations
        pattern = f"document.querySelector('#{old_id}')"
        if pattern in content:
            new_pattern = f"document.querySelector('#{new_id}')"
            count = content.count(pattern)
            content = content.replace(pattern, new_pattern)
            replacement_count += count
...
if __name__ == '__main__':
    file_path = r'c:\Users\majob\Downloads\okr_portal_app\index.html'
    
    print(f"Processing file: {file_path}")
    print("=" * 60)
    
    replacements = replace_ids(file_path)
    
    print(f"Celkový počet náhrad: {replacements}")
    print("=" * 60)
    print("Súbor bol úspešne aktualizovaný!")
